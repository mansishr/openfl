...
class ClipOptimizer(object):
    def __init__(
        self, base_optimizer: Optimizer, device, global_model_params, clip_norm: float, clip_freq: int = 1
    ):
        super().__init__()
        self.base_optimizer = base_optimizer
        self.device = device
        self.global_model_params = list(global_model_params)
    ...
    def step(self) -> None:
        ...
        # TODO: Last iteration update: if self.step_counter % self.clip_freq == 0 or last_iter:
        if self.step_counter % self.clip_freq == 0:
            # compute delta
            local_params = self.base_optimizer.param_groups[0]["params"]
            delta_params = []
            for idx in range(len(local_params)):
                delta_params.append(torch.sub(local_params[idx], self.global_model_params[idx]))
